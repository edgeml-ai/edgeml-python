...
class _ApiClient:
            ...
            try:
                ...
                if res.status_code in _RETRYABLE_STATUS_CODES and attempt < self.max_retries - 1:
                    ...
                    logger.warning(
                        ...
                        res.status_code, method, url, attempt + 1, self.max_retries, wait,
                    )
                    time.sleep(wait)
                    continue
    ...
    def post(self, path: str, payload: Optional[dict[str, Any]] = None) -> Any:
        res = self._request_with_retry(
            "POST", f"{self.api_base}{path}", json=payload or {}, headers=self._headers(),
        )
        return res.json()

    def put(self, path: str, payload: Optional[dict[str, Any]] = None) -> Any:
        res = self._request_with_retry(
            "PUT", f"{self.api_base}{path}", json=payload or {}, headers=self._headers(),
        )
        return res.json() if res.text else {}

    def patch(self, path: str, payload: Optional[dict[str, Any]] = None) -> Any:
        res = self._request_with_retry(
            "PATCH", f"{self.api_base}{path}", json=payload or {}, headers=self._headers(),
        )
        return res.json() if res.text else {}

    def delete(self, path: str, params: Optional[dict[str, Any]] = None) -> Any:
        res = self._request_with_retry(
            "DELETE", f"{self.api_base}{path}", params=params, headers=self._headers(),
        )
        return res.json() if res.text else {}

    def get_bytes(self, path: str, params: Optional[dict[str, Any]] = None) -> bytes:
        res = self._request_with_retry(
            "GET", f"{self.api_base}{path}",
    ...
    def post_bytes(self, path: str, data: bytes) -> Any:
        ...
        res = self._request_with_retry(
            "POST", f"{self.api_base}{path}", content=data, headers=headers,
        )
        return res.json()
