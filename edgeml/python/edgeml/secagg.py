...
**SecAgg+** -- :class:`SecAggPlusClient`
  ...
  Requires the ``cryptography`` package (``pip install edgeml[secagg]``).
...
def quantize(
    ...
    pre_quantized = [
        (max(-clipping_range, min(clipping_range, v)) + clipping_range) * quantizer
        for v in values
    ]
...
def _require_cryptography():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it with: pip install edgeml[secagg]"
        ) from exc
...
def generate_shared_key(
    ...
    return HKDF(
        algorithm=SHA256(), length=32, salt=None, info=info,
    ).derive(raw_shared)
        ...
        *peer_keys* maps peer index (1-based) to ``(pk1_raw, pk2_raw)``
        ...
        for idx, (_, pk2) in self._peer_public_keys.items():
            ...
            self._ss2_dict[idx] = generate_share_encryption_key(
                self._kp2.private_key_bytes, pk2,
            )
    ...
    def receive_encrypted_shares(self, shares: Dict[int, bytes]) -> None:
        ...
        *shares* maps sender index -> encrypted payload.
        ...
        for sender_idx, encrypted in shares.items():
            ...
            if shared_key is None:
                logger.warning(
                    "No shared key for peer %d, skipping share", sender_idx,
                )
                continue
