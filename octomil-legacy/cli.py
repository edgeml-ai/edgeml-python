...
def serve(
    ...
    api_base: str = (
        ...
        or "https://api.octomil.io/api/v1"
    )
...
def deploy(
    ...
    if phone:
        ...
        pair_url = f"https://octomil.io/pair?code={code}"
        # Include server URL for non-production environments
        if api_base != "https://api.octomil.io/api/v1":
            import urllib.parse
...
def init(
    ...
    save_config(config)
    click.echo(f"  Config saved to ~/.octomil/config.json")

    # 4. Print next steps
    click.echo("")
    click.echo("Next steps:")
    click.echo(f"  1. Invite team members:  octomil team add alice@{org_name.lower().replace(' ', '')}.com --role admin")
    click.echo(f"  2. Create an API key:    octomil keys create deploy-key")
    click.echo(f"  3. Set security policy:  octomil team set-policy --require-mfa")
    click.echo(f"  4. Push a model:         octomil push model.pt --name my-model --version 1.0.0")


# ---------------------------------------------------------------------------
# octomil org (informational)
# ---------------------------------------------------------------------------
