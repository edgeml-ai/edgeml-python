...
def serve(
    ...
    \b
        octomil serve gemma-1b              # default (4bit)
        octomil serve gemma-1b:8bit         # 8-bit quantization
        octomil serve llama-8b:fp16         # full precision
        octomil serve llama-3b:q4_k_m       # engine-specific quant
...
def pull(name: str, version: Optional[str], fmt: Optional[str], output: str) -> None:
    ...
    \b
        octomil pull gemma-1b                  # default variant
        octomil pull gemma-1b:8bit             # 8-bit quantization
        octomil pull sentiment-v1 --version 1.0.0 --format coreml
    """
    client = _get_client()
