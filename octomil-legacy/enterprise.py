...
# ---------------------------------------------------------------------------
# Config file helpers (~/.octomil/config.json)
# ---------------------------------------------------------------------------


def _config_path() -> Path:
    return Path.home() / ".octomil" / "config.json"


def load_config() -> dict[str, Any]:
    """Load the local Octomil config from ``~/.octomil/config.json``."""
    path = _config_path()
    if path.exists():
        try:
...
def save_config(config: dict[str, Any]) -> None:
    """Persist config to ``~/.octomil/config.json``."""
    path = _config_path()
...
def get_org_id() -> str:
    """Read org_id from config or OCTOMIL_ORG_ID env var."""
    env_id = os.environ.get("OCTOMIL_ORG_ID", "")
    if env_id:
        return env_id
    cfg = load_config()
...
class EnterpriseClient:
    ...
    Args:
        ...
        api_base: Base URL for the Octomil API (default: ``https://api.octomil.io/api/v1``).
    """
