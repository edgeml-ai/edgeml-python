    ...
    def test_jetson_on_aarch64_with_tegra(self, backend: CUDABackend) -> None:
        with patch("octomil.hardware._cuda.platform.machine", return_value="aarch64"):
            with patch(
                ...
                side_effect=lambda p: p == "/etc/nv_tegra_release",
            ):
...
class TestFullQueryDetection:
    ...
    def _run_detect(self, backend: CUDABackend, full_csv: str, cuda_version: str = "12.4"):
...
class TestSimpleQueryFallback:
    ...
    def test_simple_query_after_full_failure(self, backend: CUDABackend) -> None:
        ...
        def side_effect(*args, **kwargs):
            ...
            if cmd == ["nvidia-smi"]:
                return _smi_result("| NVIDIA-SMI 550.54.14   CUDA Version: 12.4 |\n")
            return _smi_result("", returncode=1)
