...
class TestRenderQrTerminal:
    ...
    def test_contains_block_characters(self):
        result = render_qr_terminal("https://app.octomil.io/deploy/phone?code=ABC123")
...
class TestBuildDeepLink:
    def test_basic_deep_link(self):
        url = build_deep_link(token="ABC123", host="https://api.octomil.io/api/v1")
        assert url.startswith("octomil://pair?")
        assert "token=ABC123" in url
        assert "host=https%3A%2F%2Fapi.octomil.io%2Fapi%2Fv1" in url
    ...
    def test_host_is_url_encoded(self):
        url = build_deep_link(token="T", host="https://api.octomil.io/api/v1")
        assert "host=https%3A%2F%2Fapi.octomil.io%2Fapi%2Fv1" in url

    def test_both_params_present(self):
        url = build_deep_link(token="ep_7kx", host="https://api.octomil.io/api/v1")
        ...
        assert params["host"] == ["https://api.octomil.io/api/v1"]
    ...
    def test_empty_token(self):
        url = build_deep_link(token="", host="https://api.octomil.io/api/v1")
...
class TestDeployPhoneQr:
    ...
    def test_deploy_phone_qr_payload_is_deep_link(self, mock_open, monkeypatch):
        ...
        assert "api.octomil.io" in params["host"][0]
