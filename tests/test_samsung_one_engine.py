...
class TestSamsungOneEngine:
    ...
    def test_detect_with_onert(self) -> None:
        with patch("octomil.engines.samsung_one_engine._has_onert", return_value=True):
            assert self.engine.detect() is True

    def test_detect_without_onert(self) -> None:
        with patch("octomil.engines.samsung_one_engine._has_onert", return_value=False):
            assert self.engine.detect() is False

    def test_detect_info_available(self) -> None:
        with (
            patch("octomil.engines.samsung_one_engine._has_onert", return_value=True),
            patch(
    ...
    def test_create_backend_custom_backend(self) -> None:
        with patch("octomil.engines.samsung_one_engine._has_onert", return_value=True):
            backend = self.engine.create_backend("model.nnpackage", backend="npu")
...
class TestSamsungOneBackend:
    def test_list_models(self) -> None:
        with patch("octomil.engines.samsung_one_engine._has_onert", return_value=True):
            backend = SamsungOneEngine(backend="cpu").create_backend("model.nnpackage")
